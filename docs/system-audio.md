...
By default Sotto transcribes your microphone only. To also transcribe the other
...
4. In Sotto: Settings → Audio → Microphone → **BlackHole 2ch**.
...
- **Stereo Mix** (if your driver exposes it): enable in Sound settings →
  Recording, select it in Sotto.
- [VB-CABLE](https://vb-audio.com/Cable/): set playback to CABLE Input, select
  CABLE Output as Sotto's microphone.

Native WASAPI loopback capture is on the roadmap.

## Linux — PulseAudio/PipeWire

Select the `monitor` source of your output device as Sotto's microphone
(`pavucontrol` → Recording tab while Sotto is listening).
